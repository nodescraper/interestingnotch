	...
	@MainActor func setRelative(delta: Float) {
		Task { @MainActor in
			...
			if ok {
                ...
                InterestingViewCoordinator.shared.toggleSneakPeek(status: true, type: .brightness, value: CGFloat(current), targetScreenUUID: targetUUID)
			} else {
				refresh()
			}
		}
	...
	func setAbsolute(value: Float) {
		...
		Task { @MainActor in
			...
			if ok {
                ...
                let targetUUID = await brightnessTargetUUID()
                InterestingViewCoordinator.shared.toggleSneakPeek(status: true, type: .brightness, value: CGFloat(clamped), targetScreenUUID: targetUUID)
			} else {
				refresh()
			}
		}
