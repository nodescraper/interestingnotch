...
final class AudioCaptureManager: ObservableObject {
    ...
    private let fftQueue = DispatchQueue(label: "com.interestingnotch.audiocapture.fft", qos: .userInitiated)
    private let ioQueue = DispatchQueue(label: "com.interestingnotch.audiocapture.io", qos: .userInteractive)
    private let lifecycleQueue = DispatchQueue(label: "com.interestingnotch.audiocapture.lifecycle", qos: .userInitiated)
    ...
    private func startCaptureOnLifecycleQueue(pids: [pid_t]) {
        ...
        let aggregateUID = "com.interestingnotch.audiotap.\(UUID().uuidString)"
        let aggregateDescription: [String: Any] = [
            kAudioAggregateDeviceNameKey: "InterestingNotchTapAggregate",
    ...
    private func performPermissionProbeOnLifecycleQueue() -> Bool {
        ...
        tapDescription.name = "InterestingNotch Audio Permission Probe"
        tapDescription.muteBehavior = .unmuted
        tapDescription.isPrivate = true
        ...
        let aggregateUID = "com.interestingnotch.permissionprobe.\(UUID().uuidString)"
        let aggregateDescription: [String: Any] = [
            kAudioAggregateDeviceNameKey: "InterestingNotch Audio Permission Probe",
            kAudioAggregateDeviceUIDKey: aggregateUID,
            kAudioAggregateDeviceMainSubDeviceKey: "",
            kAudioAggregateDeviceIsPrivateKey: 1,
            kAudioAggregateDeviceIsStackedKey: 0,
