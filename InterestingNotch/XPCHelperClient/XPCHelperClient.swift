...
final class XPCHelperClient: NSObject {
    ...
    private var lunarListener: InterestingNotchXPCHelperLunarListener?
    ...
    @MainActor
    private func ensureRemoteService(needsListener: Bool = false) -> RemoteXPCService<InterestingNotchXPCHelperProtocol> {
        ...
        let service = RemoteXPCService<InterestingNotchXPCHelperProtocol>(
            connection: conn,
            remoteInterface: InterestingNotchXPCHelperProtocol.self
        )
    ...
    @MainActor
    private func getRemoteService() -> RemoteXPCService<InterestingNotchXPCHelperProtocol>? {
        remoteService
    }

    private func makeLunarListenerInterface() -> NSXPCInterface {
        let interface = NSXPCInterface(with: (any InterestingNotchXPCHelperLunarListener).self)
        interface.setClasses(
            NSSet(array: [BNLunarBrightnessEvent.self]) as! Set<AnyHashable>,
            for: #selector(InterestingNotchXPCHelperLunarListener.lunarEventDidUpdate(_:)),
            argumentIndex: 0,
            ofReply: false
        )
        return interface
    ...
    nonisolated func startLunarEventStream(listener: InterestingNotchXPCHelperLunarListener) async -> Bool {
        await MainActor.run {
